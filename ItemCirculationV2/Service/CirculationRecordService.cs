...
using ItemCirculationV2.ViewModels;
        ...
        public List<CirculationView> LoanItem(IEnumerable<string> uidList, Student student)
        {
            ...
            using (var db = new MySqlDbContext())
            {
                foreach (var uid in uidList)
                {
                    ...
                    if (item == null)
                    {
                        circulationRecord.ActionId = db.Action.Join(db.ActionType, x => x.ActionTypeId, y => y.Id, (x, y) => new { x.Id, x.ActionCode, y.ActionTypeCode }).ToList().Single(x => x.ActionCode == ItemCirculation.Data.Enum.Action.异常借出.GetHashCode() && x.ActionTypeCode == ItemCirculation.Data.Enum.ActionType.未知设备.GetHashCode()).Id;
                    }
                    else
                    {
                        ...
                        circulationRecord.ActionId = entity.ExecuteResult ? db.Action.ToList().Single(x => x.ActionCode == ItemCirculation.Data.Enum.Action.正常借出.GetHashCode()).Id : db.Action.Join(db.ActionType, x => x.ActionTypeId, y => y.Id, (x, y) => new { x.Id, x.ActionCode, y.ActionTypeCode }).ToList().Single(x => x.ActionCode == ItemCirculation.Data.Enum.Action.异常借出.GetHashCode() && x.ActionTypeCode == ItemCirculation.Data.Enum.ActionType.重复借出.GetHashCode()).Id;
        ...
        public List<CirculationView> ReturnItem(IEnumerable<string> uidList, Student student)
        {
            ...
            using (var db = new MySqlDbContext())
            {
                foreach (var uid in uidList)
                {
                    ...
                    if (item == null)
                    {
                        circulationRecordEntity.ActionId = db.Action.Join(db.ActionType, x => x.ActionTypeId, y => y.Id, (x, y) => new { x.Id, x.ActionCode, y.ActionTypeCode }).ToList().Single(x => x.ActionCode == ItemCirculation.Data.Enum.Action.异常归还.GetHashCode() && x.ActionTypeCode == ItemCirculation.Data.Enum.ActionType.未知设备.GetHashCode()).Id;
                    }
                    else
                    {
                        entity.ItemNumber = item.ItemNumber;
